...
def get_chroma():
    ...
    col    = client.get_or_create_collection("bank_customers", embedding_function=ef)

    # Sync: upsert any customers not yet in the collection
    all_custs = load_customers()
    existing_ids = set(col.get(include=[])["ids"])
    missing = [c for c in all_custs if c["id"] not in existing_ids]

    if missing:
        BATCH = 100   # upsert in batches to avoid memory spikes
        for i in range(0, len(missing), BATCH):
            batch = missing[i : i + BATCH]
            col.upsert(
                ids       = [c["id"] for c in batch],
                documents = [_customer_to_doc(c) for c in batch],
                metadatas = [{"name": c["name"], "risk_tier": c["risk_tier"]} for c in batch],
            )
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    def render_chat_msg(role, content):
        ...
        bg      = f"{PRIMARY}08" if role == "user" else SURFACE
        border  = f"{PRIMARY}20" if role == "user" else BORDER
        align   = "flex-end" if role == "user" else "flex-start"
        st.markdown(
            f"<div style='display:flex;align-items:flex-start;gap:10px;"
            f"justify-content:{align};margin-bottom:10px'>"
            f"<div style='width:32px;height:32px;border-radius:9px;flex-shrink:0;"
            f"background:{border};display:flex;align-items:center;"
            f"justify-content:center;font-size:1rem;order:{'1' if role=='user' else '0'}'>{avatar}</div>"
            f"<div style='background:{bg};border:1px solid {border};border-radius:12px;"
            f"padding:0.7rem 1rem;max-width:85%;font-size:0.9rem;color:{DARK};line-height:1.55'>"
            f"{content}</div></div>",
            unsafe_allow_html=True,
        )
    ...
    if st.session_state.pending_q:
        ...
        render_chat_msg("user", question)
        with st.spinner("Analysing..."):
            answer = ask_rag(question)
        render_chat_msg("assistant", answer)
    ...
    if user_input:
        ...
        render_chat_msg("user", user_input)
        with st.spinner("Analysing..."):
